...
public:
  ...
  align(const TargetPointCloud& target, const SourcePointCloud& source, const TargetTree& target_tree, const Eigen::Isometry3d& init_T = Eigen::Isometry3d::Identity()) const {
    ...
    auto result = optimizer.optimize(target, source, target_tree, rejector, criteria, reduction, init_T, factors, general_factor);
    ...
    if (factors.empty()) {
      result.source_inlier_ratio = 0.0;
      return result;
    }

    const size_t num_stat_inliers = inlier_max_dist_sq >= 0.0
      ? std::count_if(factors.begin(), factors.end(), [&](const auto& factor) { return factor.stat_inlier(inlier_max_dist_sq); })
      : result.num_inliers;
    result.source_inlier_ratio = static_cast<double>(num_stat_inliers) / static_cast<double>(factors.size());
...
public:
  ...
  double inlier_max_dist_sq = -1.0; ///< Independent threshold for source-side inlier ratio. Negative means fallback to rejector-based inliers.
